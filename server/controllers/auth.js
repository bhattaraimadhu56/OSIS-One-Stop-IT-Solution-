...
exports.signup = (req, res) => {
  ...
  // using .exec function we can do without using it as well simply writing by comma
...
exports.signin = (req, res) => {
  ...
  User.findOne({ email }, (err, userData) => {
    // here userData is all information from the database
    ...
    if (err || !userData) {
      ...
      bcrypt.compare(req.body.password, userData.password, (err, result) => {
        // if req.body.password is matched with database password userData.password
        if (err) {
        ...
        if (result) {
          ...
          const token = jwt.sign(
            ...
            {
              // Sending information from databse to token userData._id means from database data
              t_id: userData._id,
              // t_name: userData.name,
              t_email: userData.email,
              t_owner: userData.owner,
          ...
          const { _id, cname, email, role } = userData; // here userData is all information from the database

          res.json({
            // send token and use information to frontend
...
exports.requireSignin = expressJwt({
  ...
  // by default all information of token comes under req.user
});
